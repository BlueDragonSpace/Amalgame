...
#idle, move, flinch, death
var art_state = 'idle' #too lazy for state machine, this will do
...
@export var STR: float = 140.0 #how powerfully you can swing your weapon
@export var MAX_SWING : float = 18.0 #how fast you can swing at max (and in turn, max damage)
...
signal player_hit #actually, player hp change, not hit. (bleh, bad code)
@export var max_HP: int = 5
@export var HP: int = max_HP: #hphphphphphphhphphphphphphph
	set(new_hp):
		HP = new_hp
		player_hit.emit()
...
# Please.
func _ready() -> void:
	connect("player_dead", Music.pitch_down)
	Global.Player = self
# alright continue on
...
func _process(_delta: float) -> void:
	...
	#ART ANIMATING
	if art_state == 'flinch' and not Art.is_playing():
		Art.play('idle')
		art_state = 'idle'
	elif Input.is_action_pressed('move_left') or Input.is_action_pressed('move_right') and art_state != 'flinch' and art_state != 'death':
		Art.play('move')
		art_state = 'move'
	else:
		Art.play('idle')
...
func _physics_process(delta: float) -> void:
	...
	#reset position
	if Input.is_action_just_pressed("debug3") and Global.dev_mode:
